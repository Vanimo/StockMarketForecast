...
def createTag(timeStamp, method):
    addZeroHour = ''
    addZeroDay = ''
    addZeroMonth = ''
    if (int(timeStamp[3]) < 10):
        addZeroHour = '0'
    if (int(timeStamp[2]) < 10):
        addZeroDay = '0'
    if (int(timeStamp[1]) < 10):
        addZeroMonth = '0'
    stamp = str(timeStamp[0]) + "-" + addZeroMonth + str(timeStamp[1]) + "-" + addZeroDay + str(timeStamp[2])
    if method == "byDay":
        return stamp
    if method == "byHour":
        return stamp + "_" + addZeroHour + str(timeStamp[3])
    return stamp
